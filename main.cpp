...
int main(int argc, char* argv[])
{
    ...
    auto cli = (
                ...
                | (command("build").set(selected, mode::build),
                    values("file", infiles)
                        .if_missing([]{ std::cout << "You need to provide at least one filename !" << std::endl; })
                    ,
                    required("-o", "--out") & value("outfile", output_bytecode_file)
                        .if_missing([]{ std::cout << "You need to provide the name of the output file !" << std::endl; })
                    ,
                ...
                | (command("exec").set(selected, mode::exec),
                    value("file", input_file)
                        .if_missing([]{ std::cout << "You need to provide the name of the file to execute !" << std::endl; })
                    ,
                ...
                | (command("tests").set(selected, mode::tests),
                    (option("-v", "--verbose").set(verbose) % "Print detailed report"),
                    (option("-i", "--interactive").set(interactive) % "Start a CLI to be able to execute a test instruction per instruction")
                   ),
                any_other(wrong)
                );
    ...
    else
    {
        ...
        std::cout << "Usage:" << std::endl
                  << usage_lines(cli, argv[0]) << std::endl
                  << "Options:" << std::endl
                  << documentation(cli) << std::endl
                  ;
    }
