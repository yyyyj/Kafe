...
namespace kafe
{
    ...
    enum Instruction
    {
        ...
        INST_DECL_SEG  = 0x11,  // 0x11 [segment name size on 1 byte] [segment] [position on 2 bytes]
        INST_STORE_VAR = 0x12,  // 0x12 ; store the value at stack[-2] in stack[-1]
        INST_PUSH_VAR  = 0x13,  // 0x13 [size of the name of the var on 1 byte] [var] ; push its value on the stack
        INST_JUMP      = 0x14,  // 0x14 [segment name size on 1 byte] [segment]
        INST_JUMP_IF   = 0x15,  // 0x15 [size of the segment name on 1 byte] [segment] ; jump if stack[-1] compares to true
        INST_JUMP_IFN  = 0x16,  // 0x16 [size of the segment name on 1 byte] [segment] ; jump if stack[-1] compares to false
